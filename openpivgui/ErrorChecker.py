...
def check_PIVprocessing(self): # check number of images, image types, and window sizing
    self.p = self
    '''Error checking'''
...
def check_processing(self): # check for threads
    ...
    if checker != 0:
        raise Exception(message) # raising errors did not work in try statement for some reason

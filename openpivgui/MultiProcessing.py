...
class MultiProcessing(piv_tls.Multiprocesser):
    ...
    def __init__(self, params):
        ...
        # disassociate the GUI selection from
    ...
    def process(self, args):
        ...
        if self.p['evaluation_method'] == 'Direct Correlation':
            ...
            x, y, u, v = piv_scl.uniform(
                x, y, u, v, scaling_factor=self.p['scale'])
            if self.p['smoothn_each_pass'] == True:
                u = smoothn(u)
                v = smoothn(v)
                print(
                    'Finished smoothning results for image pair: {}.'.format(counter+1))
            ...
            np.savetxt(self.save_fnames[counter],
                       out.T, fmt='%8.4f', delimiter=delimiter)
            print('Processed image pair: {}.'.format(counter+1))
